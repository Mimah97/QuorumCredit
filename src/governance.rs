...
use crate::helpers::{
    add_slash_balance, borrower_registration_time, config, deduct_slash_balance,
    get_active_loan_record, get_latest_loan_record, require_admin_approval, require_not_paused,
};
use crate::insurance;
use crate::types::{
    DataKey, LoanStatus, RedistributionRule, SlashRecord, SlashVoteRecord, TimelockAction,
...
fn execute_slash(env: &Env, borrower: &Address) -> Result<(), ContractError> {
    let cfg = config(env);
    check_borrower_immunity(env, borrower, &cfg)?;
    ...
    let insurance_part = total_slashed * crate::types::SLASH_TO_INSURANCE_BPS as i128 / BPS_DENOMINATOR;
    let distributable = total_slashed - insurance_part;
    route_slashed_funds(env, &loan.token_address, &vouches, distributable, cfg.redistribution_rule.clone());
